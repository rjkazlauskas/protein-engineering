...
Solving this equation iteratively using a Python script in the supporting information yields _c_ = 0.295. (Less elegantly, one can also try different values of $c$ with $ee_p = 92\%$ in the web tool cited above until the calculated enantioselectivity matches 35.) If you try to solve @eq-req_iterate for _c_ when $ee_p$ is a higher value of 95\% ee, you get a negative value for _c_ indicating that is it impossible. An enantioselectivity of 35 is too low to ever yield product with 95% ee.
...
**Code Block S8.1.** Python script to calculate the enantioselectivity of a reaction using @eq-E_eep.

```python
#! /usr/bin/env python
# coding: utf-8
# copy and save as E_from_eep.py
# also run the command below in terminal
# sudo chmod +x E_from_eep.py
"""
This script calculates the enantioselctivity, E, of a
kinetic resolution from the measured values of percent
conversion and enantiomeric excess of the product in percent.
"""
import math  # load math functions needed to calculate logarithms

# enter values for percent conversion and enantiomeric excess of the product
percentconv = int(input("Enter the conversion in % (example = 25): "))
percenteep = int(input("Enter the ee of the product in % (example = 90): "))

# calculate enantioselectivity and round to one digit after decimal
def ecalc(percentconv,percenteep):
  conv = percentconv/100.0 # convert percent to a fraction between 0 & 1
  eep = percenteep/100.0
  E = round((math.log(1-conv*(1+eep)))/(math.log(1-conv*(1-eep))),1)
  # math.log(x) calculates the natural logarithm of x
  return E
 # display result
print('If the enantiomeric excess of the product is',percenteep,'\b% ee at', percentconv,'\b% conversion, then the enantioselectivity is', ecalc(percentconv,percenteep),'\b.')
```
